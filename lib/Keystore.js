...
var Keystore = function () {
  ...
  _createClass(Keystore, [{
    ...
    value: function getAddressesFromMnemonic(password, accountId, iteration) {
      var account = this.getAccount({ id: accountId, type: this.mnemonicType });
    ...
    value: function _generateAddresses(password, account) {
      ...
      var keyIndexStart = iteration * this.addressesCountToGenerate;
      var keyIndexEnd = keyIndexStart + this.addressesCountToGenerate;
    ...
    value: function _reEncryptData(password, newPassword) {
      ...
      this.accounts.forEach(function (account) {
        ...
        Object.keys(encrypted).forEach(function (key) {
          ...
          if (encryptedItem) {
            var decryptedItem = _this._decryptData(encryptedItem, password, isPrivateKey);

            newEncrypted[key] = _this._encryptData(decryptedItem, newPassword);
          } else {
            newEncrypted[key] = encryptedItem;
          }
        });

        _this._setAccount(account, { encrypted: newEncrypted });
      });
    }
  }], [{
    key: 'isMnemonicValid',
    ...
    key: 'isHashStringValid',
    value: function isHashStringValid(hash, hashLength) {
